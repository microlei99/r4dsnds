...
class TradeImage {
	...
	{
		return new TradeImage($image, $config);
	}
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(TradeImage::$allowed_types[$image_info[2]]))
		...
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => TradeImage::$allowed_types[$image_info[2]],
    ...
    {
        if(!is_numeric($scale))
            throw new CException('scale invalid', $scale);
